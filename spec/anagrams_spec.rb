...
describe('#anagrams') do
  ...
  it('will check for palindromes') do
    ...
    expect(user_words.palindromes()).to(eq("These words are palindromes"))
  end

  it('will require words with vowels') do
    user_words = Anagram.new('why hmph', 'she is an unbelievable diva')
    expect(user_words.is_word()).to(eq("You need to input actual words!"))
  end

  it('will check for antigrams') do
    user_words = Anagram.new('hi', 'bye')
    expect(user_words.antigrams()).to(eq("These words have no letter matches and are antigrams!"))
  end
